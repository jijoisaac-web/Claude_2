...
- Both draw on the chart (purple arrows/dashed line for CHoCH, green/red arrows for BOS, amber circles for sweeps) merged into the same marker set as the existing disclosed-deals markers, plus a cards summary (current trend, last CHoCH, last sweep) and a combined chronological table.
- Verified against a hand-built synthetic candle series (clean uptrend → engineered reversal → downtrend → wick-and-rally) before shipping: correctly fired BOS through the uptrend, CHoCH exactly at the engineered reversal, BOS through the downtrend, and picked up the sweep-then-CHoCH combo at the end without it being specifically targeted — the classic combined SMC setup emerging from the general logic rather than being hard-coded.
- Investigated connecting TradingView instead of building this natively — confirmed via their own docs that there's no public API for pulling indicator/pattern data out of TradingView (widgets are display-only and can't be fed your own data or scraped; the free Charting Library runs the other direction, rendering *your* data with no TradingView indicators included). Native build was the only path that could integrate with the Screener/Backtest/Conviction Scan pipeline anyway.
...
- Fixed by walking the *entire* AI response object looking for either an object that already contains the expected fields, or a string anywhere inside it that parses into one — instead of assuming the content lives at exactly `r.response` or `r.result` as a string. Applied to all four AI endpoints that shared the identical flawed line: `report/[symbol].js`, `finai/[symbol].js`, `explain/[symbol].js`, and `news.js` (the last of these hadn't yet thrown a visible error, but had the same latent bug).
- Failure diagnostics upgraded too: if extraction still comes up empty, the error message now includes a snippet of the full JSON-stringified response (not just the old text field), so any further failure shows the actual shape Workers AI returned instead of another "[object Object]" dead end.
...
- Sanity line compares fund NAV 3M vs holdings-weighted 3M — a large gap flags cash/debt allocation or stale holdings data
- New `/api/mf/holdings` best-effort proxy (unofficial source, resilient JSON probing, 6h cache); holdings matched to the ~750-share universe by normalized name
