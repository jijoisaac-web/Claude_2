/* BestDeals server — compares live prices via official APIs.
...
async function searchEbay(q, c) {
  if (!process.env.EBAY_CLIENT_ID || !process.env.EBAY_CLIENT_SECRET || !c.ebay) return [];
...
/* ---------------- provider: Best Buy API (free key, US only) ---------------- */
async function searchBestBuy(q, c) {
  if (!process.env.BESTBUY_API_KEY || !c.bestbuy) return [];
  const terms = q.trim().split(/\s+/).map(w => `search=${encodeURIComponent(w)}`).join("&");
  const url = `https://api.bestbuy.com/v1/products(${terms})?apiKey=${process.env.BESTBUY_API_KEY}&format=json&sort=salePrice.asc&pageSize=8&show=name,salePrice,regularPrice,url,image,onSale`;
...
/* ---------------- provider: SerpAPI Google Shopping (free tier) ----------------
 * One call returns prices from many stores: Walmart, Target, Amazon sellers, etc. */
async function searchSerpApi(q, c) {
  if (!process.env.SERPAPI_KEY) return [];
  const url = `https://serpapi.com/search.json?engine=google_shopping&q=${encodeURIComponent(q)}&gl=${c.gl}&api_key=${process.env.SERPAPI_KEY}&num=15`;
  const r = await fetch(url);
...
/* ---------------- demo data (used when no API keys configured) ---------------- */
function hashCode(s) { let h = 0; for (let i = 0; i < s.length; i++) { h = (h << 5) - h + s.charCodeAt(i); h |= 0; } return Math.abs(h); }

function demoResults(q, c) {
  const h = hashCode(q.toLowerCase());
  const baseUSD = 20 + (h % 980);           // deterministic per query
  ...
  return stores.map((s, i) => {
    const jitter = 1 + (((h >> (i * 3)) % 13) - 6) / 100; // ±6%
...
app.get("/api/search", async (req, res) => {
  ...
  const hasKeys = !!(process.env.SERPAPI_KEY || process.env.EBAY_CLIENT_ID || process.env.BESTBUY_API_KEY);
  let results = [], errors = [];

  if (hasKeys) {
    const settled = await Promise.allSettled([searchSerpApi(q, c), searchEbay(q, c), searchBestBuy(q, c)]);
  ...
  res.json({ query: q, country, currencySymbol: c.sym, demo, errors, best: results[0], results });
});
...
app.listen(PORT, () => {
  const hasKeys = !!(process.env.SERPAPI_KEY || process.env.EBAY_CLIENT_ID || process.env.BESTBUY_API_KEY);
  console.log(`\n✅ BestDeals running at http://localhost:${PORT}`);
  console.log(hasKeys ? "🔑 API keys detected — LIVE prices enabled." : "🧪 DEMO mode — no API keys found. See README.md to enable live prices.\n");
});
